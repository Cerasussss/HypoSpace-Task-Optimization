...
class Benchmark3D:
    ...
    def create_prompt(self, observations, prior_structures=None) -> str:
        ...
        rag_context = None
        if self.rag_module and isinstance(observations, str):
            try:
                # Format observation for RAG module
                obs_dict = [{"observation": observations, "string": observations}]
                # Use diverse retrieval to get more varied examples
                similar_examples = self.rag_module.retrieve_diverse_examples(obs_dict, top_k=3)
                rag_context = self.rag_module.format_examples_for_prompt(similar_examples, task_type="3d")
    ...
    def parse_llm_response(self, response: str) -> Optional[Structure3D]:
        ...
        try:
            ...
            for line in lines[structure_start:]:
                ...
                if not line or line.startswith('Note:') or line.startswith('Reasoning:') or line.startswith(
                        'Explanation:'):
                    continue
        ...
        for i in range(n_queries):
            ...
            if structure:
                ...
                if s_hash not in unique_hashes:
                    ...
                    prior_structures = unique_structures[-10:] if len(
                        unique_structures) > 10 else unique_structures.copy()
    ...
    def run_benchmark(
        ...
        for idx in range(start_idx, len(sampled_sets)):
            ...
            if verbose:
                ...
                n_obs = 1 if 'observation' in obs_set and isinstance(obs_set['observation'], str) else obs_set.get(
                    'n_observations', 1)
...
def setup_llm(llm_type: str, **kwargs) -> LLMInterface:
    ...
    if llm_type == "openai":
        ...
        return OpenAILLM(
            ...
            max_tokens=kwargs.get('max_tokens', 40960)
        )
        ...
        if not model_path:
            raise ValueError("Qwen local model path required")
...
def main():
    ...
    parser.add_argument("--observation-type", type=str, default="top", choices=["top"],
    ...
    max_tokens = config.get('llm', {}).get('max_tokens', 40960)  # 默认最大token数
    ...
    # Initialize benchmark with config for RAG support
    benchmark = Benchmark3D(args.dataset, config)
    ...
    # Set up LLM with additional parameters
    llm_kwargs = {
        'model': model,
        'api_key': api_key,
