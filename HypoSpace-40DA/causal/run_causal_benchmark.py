    ...
    def __init__(self, complete_dataset_path: Optional[str] = None,
        ...
        if complete_dataset_path:
            ...
            if n_observations_filter:
                ...
                print(
                    f"Stage 1: Filtered to {len(stage1_filtered)} observation sets with n_observations in {n_observations_filter}")
            ...
            if gt_filter:
                if gt_filter[1] is not None:  # Range filter (min, max)
                    ...
                    print(
                        f"Stage 2: Filtered to {len(self.filtered_observation_sets)} observation sets with n_compatible_graphs in [{min_gt}, {max_gt}]")
                    ...
                    print(
                        f"Stage 2: Filtered to {len(self.filtered_observation_sets)} observation sets with n_compatible_graphs in {allowed_values}")
...
def main():
    ...
    # Initialize benchmark with filters and config
